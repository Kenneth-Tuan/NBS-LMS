...
const itemOptions = computed(() =>
  (otherApplicationItems || []).map((item) => ({ label: item.name, value: item.id }))
);
const selectedItem = computed(() =>
  (otherApplicationItems || []).find((i) => i.id === otherApplicationForm.other_application_id)
);
...
<template>
    <div class="u-p-1rem u-w-full">
      <div class="u-bg-white u-rounded-0.5rem u-p-1.5rem u-shadow-md">
        ...
        <a-form ref="otherFormRef" layout="vertical" :model="otherApplicationForm" @finish="handleSubmit">
          <ApplicantInfo />
          ...
          <a-row :gutter="16">
            <a-col :span="12">
              <a-form-item v-bind="otherApplicationSchema.other_application_id" name="other_application_id">
                <a-select
              ...
              <a-form-item label="附件下載">
                <div v-if="selectedItem">
                  <a-space direction="vertical" style="width: 100%">
                    <a-empty v-if="normalizeAttachments(selectedItem.attachments).length === 0" description="無附件" />
                    <a-button
                      v-for="(att, idx) in normalizeAttachments(selectedItem.attachments)"
                      :key="idx"
                      type="default"
                ...
                <div v-else>
                  <a-typography-text type="secondary">請先選擇申請項目</a-typography-text>
                </div>
              </a-form-item>
            </a-col>

            <a-col :span="12">
              <a-form-item v-bind="otherApplicationSchema.attachments" name="attachments">
                <a-upload
                  list-type="picture"
                  v-model:file-list="otherApplicationForm.attachments"
                  :before-upload="beforeUpload"
                  :custom-request="async () => { await processFileList(otherApplicationForm.attachments) }"
                  :disabled="uploading"
                >
                  <a-button>
                  ...
                  <template #itemRender="{ file, actions }">
                    ...
                    <span>
                      <a-button type="link" @click="() => actions.remove()">删除</a-button>
                    </span>
                  </template>
                </a-upload>
              </a-form-item>
